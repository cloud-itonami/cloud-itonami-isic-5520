(ns campgroundops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (CampgroundOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the CampgroundGovernor (:govern)
  ...
  human operator/campground manager. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [campgroundops.advisor :as advisor]
            [campgroundops.governor :as governor]
...
(defn build
  ...
  `campgroundops.store/Store`).
  opts:
    :advisor      -- a `campgroundops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CampgroundOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; CampgroundGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
